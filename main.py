...
def generate():
    ...
    if len(female_va_entry.get()) == 0:
        ...
        messagebox.showinfo("Success","Well You are a certified weeb \n QRCODE saved")
...
def main():
    root = tk.Tk()
    messagebox.showinfo("Ora Gumbare Gumare","Its difficult to choose just one, yet try giving your best 🤡")
    pygame.init()
    pygame.mixer.init()
    pygame.mixer.music.load(r'music1.mp3')
    pygame.mixer.music.play(-1,6)
    root.wm_attributes('-topmost',True)
    root.geometry("675x605+185+60")
    root.resizable(0,0)
    root.title("Otaku-Profile QR")
    ...
    btn = tk.Button(root,text="Generate QR profile",font=("arial",14,"bold"),bd=5,fg="white",bg="blue",command=generate).place(x=465,y=555)
    root.mainloop()
